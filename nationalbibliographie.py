"""
Programm zur Ausgabe von Neuerscheinungslisten aus dem Katalog der DNB
"""
...
class Normdaten(BaseModel):
    """
    Gibt Name von Person oder Ort sowie GND-ID an
    "relevant" wird künftig dann bestimmen, ob das Schlagwort als für Bayerisch-Schwaben interessant 
    hervorgehoben wird
    """
    name : Optional[str] = ""
    id : Optional[str] = ""
...
def find_subfields(datafield,subfield_id):
    ...
    for subfield in subfields:
        ...
        value=subfield.text
        if key == subfield_id:
            value = ud.normalize('NFC',value)
            r.append(value)
    return r


def find_subfields_with_ind(datafield,subfield_id):
    """
    Returns a list of all subfields of a datafield in a MARCXML 
    Document with the same subfield number
    Variant: the list elements are not subfields as strings, 
    but tuples subfield, ind 1 of field, ind 2 of field
    """
    r=[]
    subfields=datafield.findall("{*}subfield")
...
#            value = nationalbibliographie_config.umlaut_nach_unicode(value)
            value_tuple = (value, ind1, ind2)
            r.append(value_tuple)
    return r
...
def lade_umfangsangaben(eintrag):
    ...
    """
    umfangsangaben = ""
    umfangsangaben, seitenzahl, illustrationen, buchformat = "", "", "", ""
    ...
    if datafields:
        ...
        subfields = find_subfields(datafields[0], "c")
        if subfields:
            buchformat = subfields[0]
            if (seitenzahl and buchformat) or (illustrationen and buchformat):
...
#            print(schlagwort_tupel)
    ...
    for i in range(0, len(schlagwortliste)+1): 
        if schlagwortanzeige != "" and schlagwortanzeige[-4:] != " || ":
            schlagwortanzeige = schlagwortanzeige + " || "
        for j in range(0, len(schlagwortliste)+1): 
            for k in schlagwortliste:
                if len(k) > 0:
                    if k[1] == str(i) and k[2] == str(j):
                        schlagwortanzeige = schlagwortanzeige + " ; " + k[0]
                        break


    schlagwortanzeige = schlagwortanzeige[3:-4]
...
def pdf_erzeugen(bibliographie_nummer, bibliographie_liste):
    ...
    class PDF(FPDF):
        def header(self):
            self.set_font("Garamond", size=10, style="B")
            self.cell(0, 10, f"{bibliographie_nummer}                                                                                 Seite {self.page_no()} von {{nb}}", align="R")
            self.ln()
                ...
                for sachgruppennummer in sachgruppennummernliste:
                    ...
                    for katalogisat in bibliographie_liste:
                        if sachgruppennummer in katalogisat.ddc_zum_sortieren and katalogisat.schon_abgedruckt is False and sachgruppenliste[sachgruppennummer][1] is True:
                            ...
                            cols.write(text=katalogisat.schlagwortliste)
                            self.set_font(style="")
                            cols.ln()
                            cols.ln()
    ...
    pdf.add_font("Garamond", style="i", fname=r"C:\windows\fonts\garait.ttf")
